...
export const Contact = () => {
    ...
    return (
        <section className="md:py-24 sm:py-16 py-10 bg-zinc-100/50 dark:bg-zinc-900/40 backdrop-blur-md relative overflow-hidden" id="contact">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
                    <div data-aos="fade-right">
                        <div className="text-center md:text-left">
                            <span className="text-indigo-500 font-mono tracking-[0.3em] uppercase text-xs mb-4 block">Get In Touch</span>
                            <h2 className="text-4xl md:text-6xl font-black text-foreground uppercase tracking-tighter mb-8">
                                Let's Work <br /> <span className="text-indigo-500">Together.</span>
                            </h2>
                        </div>
                    ...
                    <div className="relative" data-aos="fade-left">
                        <div className="bg-white dark:bg-[#080f1a]/90 border border-indigo-100 dark:border-indigo-900/20 p-4 sm:p-8 rounded-2xl backdrop-blur-sm relative z-10 shadow-sm dark:shadow-none">
                            <form onSubmit={handleSubmit} className="space-y-5">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <textarea
                                        ...
                                        rows={4}
                                        className="w-full bg-background border border-border rounded-2xl px-5 py-4 text-foreground focus:outline-none focus:border-indigo-500/50 transition-all resize-none placeholder:text-muted-foreground"
                                ...
                                <button
                                    ...
                                    className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-3 transition-all transform active:scale-[0.98] disabled:opacity-70"
                                >
                        ...
                        <div className="absolute -bottom-6 -left-6 w-32 h-32 bg-indigo-500/10 rounded-full blur-3xl -z-10"></div>
                    </div>
                </div>
            </div>
        </section>
...
const ContactInfo = ({ icon, label, value }: { icon: React.ReactNode, label: string, value: string }) => (
    <div className="inline-flex items-center gap-5 group">
        <div className="w-12 h-12 rounded-xl bg-muted dark:bg-neutral-900 border border-border dark:border-white/5 flex items-center justify-center text-indigo-500 group-hover:bg-indigo-600 group-hover:text-white transition-all duration-300">
            {icon}
        </div>
        <div>
...
const FormInput = ({ label, ...props }: any) => (
    <div className="space-y-2">
        ...
        <input
            {...props}
            className="w-full bg-background border border-border rounded-xl px-5 py-4 text-foreground focus:outline-none focus:border-indigo-500/50 transition-all placeholder:text-muted-foreground"
        />
    </div>
)